...
class DemographicData:
    ...
    def retrieve_demographics_from_hosp_adm(df: pl.DataFrame) -> pl.DataFrame:
        # Skip demographic extraction for sample data without text_value column
    ...
    def process_race(df: pl.DataFrame) -> pl.DataFrame:
        # Skip for sample data without text_value column
        if "text_value" not in df.columns:
            return df
...
class InpatientData:
    ...
    def process_drg_codes(df: pl.DataFrame) -> pl.DataFrame:
        # Skip for sample data without DRG codes
        if not df.filter(pl.col.code.str.starts_with("DRG//HCFA")).height:
            return df
    ...
    def process_hospital_admissions(df: pl.DataFrame) -> pl.DataFrame:
        # Skip for sample data without insurance column
...
class MeasurementData:
    ...
    def process_blood_pressure(bp_df: pl.DataFrame) -> pl.DataFrame:
        # Skip for sample data without text_value column
        if "text_value" not in bp_df.columns:
            return bp_df
        return (
            bp_df.with_columns(
...
class DiagnosesData:
    ...
    def prepare_codes_for_processing(df: pl.DataFrame) -> pl.DataFrame:
        # Skip for sample data without DIAGNOSIS codes
        if not df.filter(pl.col.code.str.starts_with("DIAGNOSIS//ICD//")).height:
            return df
    ...
    def convert_icd_9_to_10(icd9_df: pl.DataFrame) -> pl.DataFrame:
        # Skip for sample data without text_value column
        if "text_value" not in icd9_df.columns:
            return icd9_df
    ...
    def process_icd10(icd10_df: pl.DataFrame, vocab: list[str] | None = None) -> pl.DataFrame:
        # Skip for sample data without text_value column
        if "text_value" not in icd10_df.columns:
            return icd10_df
...
class ProcedureData:
    ...
    def prepare_codes_for_processing(df: pl.DataFrame) -> pl.DataFrame:
        # Skip for sample data without PROCEDURE codes
        if not df.filter(pl.col.code.str.starts_with("PROCEDURE")).height:
            return df
        return (
...
class MedicationData:
    ...
    def convert_to_atc(df: pl.DataFrame, vocab: list[str] | None = None) -> pl.DataFrame:
        ...
        df = (
            df.with_columns(pl.col("code").str.split("//"))
            .with_columns(
                # Handle both 2-part (MEDICATION//DrugName) and 3-part (MEDICATION//START//DrugName) codes
                pl.when(pl.col("code").list.len() >= 3)
                .then(
                    pl.when(pl.col("code").list[2] == "Administered")
                    .then(None)
                    .when(pl.col("code").list[1] == "START")
                    .then(pl.lit("MEDICATION_START"))
                    .otherwise(pl.lit("MEDICATION"))
                )
                .otherwise(pl.lit("MEDICATION"))
                .alias("code"),
                pl.when(pl.col("code").list.len() >= 3)
    ...
    def process(df: pl.DataFrame, *, num_quantiles: int = 10) -> pl.DataFrame:
        ...
        return (
            ...
            .with_columns(
                ...
                sofa_quantiles=pl.col("stay_id").replace_strict(stay_id_to_sofa, default=None),
            )
            .with_columns(
